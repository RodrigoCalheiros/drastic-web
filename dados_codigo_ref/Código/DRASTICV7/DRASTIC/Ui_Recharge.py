from qgis.PyQt import QtCore, QtGui
from qgis.core import *
from qgis.gui import *
from qgis.PyQt.QtWidgets import *


class Ui_Recharge(object):
    
    def setupUi(self, Recharge_window):
        
        # create Recharge window
        Recharge_window.setWindowModality(QtCore.Qt.ApplicationModal)
        Recharge_window.resize(450,450)
        
        # input file
        # create gridLayout
        self.gridLayout1 = QGridLayout(Recharge_window)
        self.gridLayout1.setObjectName("gridLayout1")        
        # define a groupbox to specify the input method 1
        self.groupBox0 = QGroupBox(Recharge_window)
        self.groupBox0.setObjectName("groupBox0")
        self.groupBox0.setTitle("Base")
        self.gridLayout = QGridLayout(self.groupBox0)
        self.gridLayout.setObjectName("gridLayout")
        self.gridLayout1.addWidget(self.groupBox0, 0,0,1,-1)        
        # create label in gridLayout 
        self.label = QLabel(Recharge_window)
        self.label.setObjectName("label")
        self.label1 = QLabel(Recharge_window)
        self.label1.setObjectName("label1")        
        self.label2 = QLabel(Recharge_window)
        self.label2.setObjectName("label2")               
        # define label (QWidget, row, column, QtAlignement)
        self.gridLayout.addWidget(self.label,0,0,1,1)
        self.gridLayout.addWidget(self.label1,1,0,1,1)
        self.gridLayout.addWidget(self.label2,2,0,1,1)
        # create select button to input file
        self.selectButton = QPushButton(Recharge_window)
        self.selectButton.setObjectName("selectButton")
        self.gridLayout.addWidget(self.selectButton,0,2,1,1)
        self.selectButton1 = QPushButton(Recharge_window)
        self.selectButton1.setObjectName("selectButton1")
        self.gridLayout.addWidget(self.selectButton1,1,2,1,1)        
        self.selectButton2 = QPushButton(Recharge_window)
        self.selectButton2.setObjectName("selectButton2")
        self.gridLayout.addWidget(self.selectButton2,2,2,1,1)        
        self.inputLayerCombo = QComboBox(Recharge_window)
        self.inputLayerCombo.setObjectName("inputLayerCombo")
        self.gridLayout.addWidget(self.inputLayerCombo, 0,1,1,1)
        self.inputLayerCombo1 = QComboBox(Recharge_window)
        self.inputLayerCombo1.setObjectName("inputLayerCombo1")
        self.gridLayout.addWidget(self.inputLayerCombo1, 1,1,1,1)        
        self.inputLayerCombo2 = QComboBox(Recharge_window)
        self.inputLayerCombo2.setObjectName("inputLayerCombo2")
        self.gridLayout.addWidget(self.inputLayerCombo2, 2,1,1,1)                
        # stretch to extend the widget in column 1
        self.gridLayout.setColumnStretch(1,1)     
        
        # define a groupbox to specify the input method 2
        self.groupBox4 = QGroupBox(Recharge_window)
        self.groupBox4.setObjectName("groupBox4")
        self.groupBox4.setTitle("Improvement")
        self.gridLayout4 = QGridLayout(self.groupBox4)
        self.gridLayout4.setObjectName("gridLayout4")
        self.gridLayout1.addWidget(self.groupBox4, 2,0,1,-1)   
        self.selectButton4 = QPushButton(Recharge_window)
        self.selectButton4.setObjectName("selectButton4")
        self.gridLayout4.addWidget(self.selectButton4,0,2,1,1)   
        self.inputLayerCombo4 = QComboBox(Recharge_window)
        self.inputLayerCombo4.setObjectName("inputLayerCombo4")
        self.gridLayout4.addWidget(self.inputLayerCombo4, 0,1,1,1)    
        self.label4 = QLabel(Recharge_window)
        self.label4.setObjectName("label4")  
        self.gridLayout4.addWidget(self.label4,0,0,1,1)
        self.gridLayout4.setColumnStretch(1,1)

        # define a groupbox to specify the input method 3
        self.groupBox5 = QGroupBox(Recharge_window)
        self.groupBox5.setObjectName("groupBox5")
        self.groupBox5.setTitle("Improvement precipitation")
        self.gridLayout5 = QGridLayout(self.groupBox5)
        self.gridLayout5.setObjectName("gridLayout5")
        self.gridLayout1.addWidget(self.groupBox5, 3, 0, 1, -1)
        self.selectButton5 = QPushButton(Recharge_window)
        self.selectButton5.setObjectName("selectButton5")
        self.gridLayout5.addWidget(self.selectButton5, 0, 2, 1, 1)
        self.inputLayerCombo5 = QComboBox(Recharge_window)
        self.inputLayerCombo5.setObjectName("inputLayerCombo5")
        self.gridLayout5.addWidget(self.inputLayerCombo5, 0, 1, 1, 1)
        self.label5 = QLabel(Recharge_window)
        self.label5.setObjectName("label5")
        self.gridLayout5.addWidget(self.label5, 0, 0, 1, 1)
        self.gridLayout5.setColumnStretch(1, 1)
        # define attribute "value" for precipitation
        self.labelAttrib5 = QLabel(Recharge_window)
        self.labelAttrib5.setObjectName("labelAttrib5")
        self.gridLayout5.addWidget(self.labelAttrib5, 0, 3, 1, 1)
        self.lineAttrib5 = QComboBox(Recharge_window)
        self.lineAttrib5.setObjectName("lineAttrib5")
        self.gridLayout5.addWidget(self.lineAttrib5, 0, 4, 1, 1)
        # define precipitation value
        self.labelprec = QLabel(Recharge_window)
        self.labelprec.setObjectName("labelprec")
        self.gridLayout5.addWidget(self.labelprec, 0, 5, 1, 1)
        self.lineprec = QSpinBox()
        self.lineprec.setValue(29)
        self.lineprec.stepBy(1)
        self.lineprec.setObjectName("lineprec")
        self.gridLayout5.addWidget(self.lineprec, 0, 6, 1, 1)



        # define a groupbox to specify the cell size and attribute
        self.groupBox1 = QGroupBox(Recharge_window)
        self.groupBox1.setObjectName("groupBox1")
        self.gridLayout3 = QGridLayout(self.groupBox1)
        self.gridLayout3.setObjectName("gridLayout3")
        self.gridLayout1.addWidget(self.groupBox1, 1,0,1,-1)
        # define attribute "Elevation" for precipitation
        self.labelAttrib = QLabel(Recharge_window)
        self.labelAttrib.setObjectName("labelAttrib")
        self.gridLayout3.addWidget(self.labelAttrib,0,0,1,1)
        self.lineAttrib = QComboBox(Recharge_window)
        self.lineAttrib.setObjectName("lineAttrib")
        self.gridLayout3.addWidget(self.lineAttrib,0,1,1,1)
        # define attribute for surface runoff
        self.labelAttribRunoff = QLabel(Recharge_window)
        self.labelAttribRunoff.setObjectName("labelAttribRunoff")
        self.gridLayout3.addWidget(self.labelAttribRunoff,1,0,1,1)
        self.lineAttribRunoff = QComboBox(Recharge_window)
        self.lineAttribRunoff.setObjectName("lineAttribRunoff")
        self.gridLayout3.addWidget(self.lineAttribRunoff,1,1,1,1)  
        # define attribute for evapotranspiration
        self.labelAttribEvap = QLabel(Recharge_window)
        self.labelAttribEvap.setObjectName("labelAttribEvap")
        self.gridLayout3.addWidget(self.labelAttribEvap,2,0,1,1)
        self.lineAttribEvap = QComboBox(Recharge_window)
        self.lineAttribEvap.setObjectName("lineAttribEvap")
        self.gridLayout3.addWidget(self.lineAttribEvap,2,1,1,1)        
        # define pixel size
        self.labelPix = QLabel(Recharge_window)
        self.labelPix.setObjectName("labelPix")
        self.gridLayout3.addWidget(self.labelPix, 0,3,1,1)
        self.linePix = QSpinBox()
        self.linePix.setValue(9)
        self.linePix.stepBy(1)
        self.linePix.setObjectName("linePix")
        self.gridLayout3.addWidget(self.linePix,0,4,1,1)         
        
        # define the indexs
        # create a group box
        self.groupBox = QGroupBox(Recharge_window)
        self.groupBox.setObjectName("groupBox")
        self.gridLayout2 = QGridLayout(self.groupBox)
        self.gridLayout2.setObjectName("gridLayout2")
        self.gridLayout1.addWidget(self.groupBox, 5,0,1,-1)
        # table for classes and indexes
        self.tableWidget = QTableWidget(5,3,Recharge_window)
        self.gridLayout2.addWidget(self.tableWidget,0,0,1,1)
        self.newItem = QTableWidgetItem()
        self.tableWidget.setHorizontalHeaderItem(0,self.newItem)
        self.newItem = QTableWidgetItem()
        self.tableWidget.setHorizontalHeaderItem(1,self.newItem)
        self.newItem = QTableWidgetItem()
        self.tableWidget.setHorizontalHeaderItem(2,self.newItem)         
        # set the values (intervals)
        self.line = QLineEdit("0")
        self.tableWidget.setItem(0,0,QTableWidgetItem(self.line.text()))
        self.line = QLineEdit("51")
        self.tableWidget.setItem(0,1,QTableWidgetItem(self.line.text()))
        self.tableWidget.setItem(1,0,QTableWidgetItem(self.line.text()))
        self.line = QLineEdit("102")
        self.tableWidget.setItem(1,1,QTableWidgetItem(self.line.text()))
        self.tableWidget.setItem(2,0,QTableWidgetItem(self.line.text()))
        self.line = QLineEdit("178")
        self.tableWidget.setItem(2,1,QTableWidgetItem(self.line.text()))
        self.tableWidget.setItem(3,0,QTableWidgetItem(self.line.text()))
        self.line = QLineEdit("254")
        self.tableWidget.setItem(3,1,QTableWidgetItem(self.line.text()))
        self.tableWidget.setItem(4,0,QTableWidgetItem(self.line.text()))
        self.line = QLineEdit("1000")
        self.tableWidget.setItem(4,1,QTableWidgetItem(self.line.text()))
        # set the indexes values
        self.line = QLineEdit("1")
        self.tableWidget.setItem(0,2,QTableWidgetItem(self.line.text()))
        self.line = QLineEdit("3")
        self.tableWidget.setItem(1,2,QTableWidgetItem(self.line.text()))
        self.line = QLineEdit("6")
        self.tableWidget.setItem(2,2,QTableWidgetItem(self.line.text()))
        self.line = QLineEdit("8")
        self.tableWidget.setItem(3,2,QTableWidgetItem(self.line.text()))
        self.line = QLineEdit("9")
        self.tableWidget.setItem(4,2,QTableWidgetItem(self.line.text()))
        # create a box layout to insert the buttons Add and Remove
        self.boxLayout = QVBoxLayout()
        self.boxLayout.setObjectName("boxLayout")
        # button Add
        self.buttonAdd = QPushButton(Recharge_window)
        self.buttonAdd.setObjectName("buttonAdd")
        self.boxLayout.addWidget(self.buttonAdd)
        # button Remove
        self.buttonRemove = QPushButton(Recharge_window)
        self.buttonRemove.setObjectName("buttonRemove") 
        self.boxLayout.addWidget(self.buttonRemove)
        self.gridLayout2.addLayout(self.boxLayout,0,1,-1,1)
        ## button weight
        #self.labelWeight = .QLabel(Recharge_window)
        #self.labelWeight.setObjectName("labelWeight")
        #self.boxLayout.addWidget(self.labelWeight)
        #self.lineWeight = .QSpinBox()
        #self.lineWeight.setValue(3)
        #self.lineWeight.stepBy(1)
        #self.lineWeight.setObjectName("lineWeight")
        #self.boxLayout.addWidget(self.lineWeight)
        self.gridLayout2.addLayout(self.boxLayout,0,1,-1,1)         
        
        # output file
        # create label in gridLayout
        self.label3 = QLabel(Recharge_window)
        self.label3.setObjectName("label3")
        # define label (QWidget, row, column, QtAlignement)
        self.gridLayout1.addWidget(self.label3,6,0,1,1)
        # create select button to input file
        self.selectButton3 = QPushButton(Recharge_window)
        self.selectButton3.setObjectName("selectButton3")
        self.gridLayout1.addWidget(self.selectButton3,6,2,1,1)
        self.inputLayerCombo3 = QLineEdit(Recharge_window)
        self.inputLayerCombo3.setObjectName("inputLayerCombo3")
        self.gridLayout1.addWidget(self.inputLayerCombo3, 6,1,1,1)
        # stretch to extend the widget in column 1
        self.gridLayout1.setColumnStretch(1,1)        
        
        # button Ok, Close and Help
        self.buttonBox = QDialogButtonBox(Recharge_window)
        #self.buttonBox.setMaximumSize(QtCore.QSize(200, 16777215))
        #self.buttonBox.setBaseSize(QtCore.QSize(110, 0))
        #self.buttonBox.setOrientation(QtCore.Qt.Horizontal)
        self.buttonBox.setStandardButtons(QDialogButtonBox.Cancel|QDialogButtonBox.Help|QDialogButtonBox.Ok)
        self.buttonBox.setObjectName("buttonBox")
        self.gridLayout1.addWidget(self.buttonBox, 8, 1, 1, 1)
        
        self.retranslateUi(Recharge_window)
        self.buttonBox.rejected.connect(Recharge_window.close)
                
    def retranslateUi(self, Recharge_window):
        Recharge_window.setWindowTitle('Net Recharge (R)')
        self.label.setText('Precipitation (mm/year):')
        self.label1.setText('Overland Flow (mm/year):')
        self.label2.setText('Evapotranspiration (mm/year):')
        self.label4.setText('DEM (m):')
        self.label5.setText('Precipitation (mm/y):')
        self.selectButton.setText('Browse')
        self.selectButton1.setText('Browse')
        self.selectButton2.setText('Browse')
        self.groupBox.setTitle("Ratings")
        self.tableWidget.horizontalHeaderItem(0).setText("Net Recharge(mm/year)")
        self.tableWidget.horizontalHeaderItem(1).setText("Net Recharge (mm/year)")
        self.tableWidget.horizontalHeaderItem(2).setText("Ratings")
        self.buttonAdd.setText("Add")
        self.buttonRemove.setText("Remove")
        self.label3.setText('Output file:')
        self.selectButton3.setText('Browse')
        self.selectButton4.setText('Browse')
        self.selectButton5.setText('Browse')
        #self.labelWeight.setText(.QApplication.translate('Net Recharge (R)', 'Weight:', None, .QApplication.UnicodeUTF8))
        self.labelPix.setText('Cell size:')
        self.labelprec.setText('Precipitation %:')
        self.labelAttrib5.setText('Attribute Precipitation:')
        self.labelAttrib.setText('Attribute Precipitation:')
        self.labelAttribRunoff.setText('Attribute Surface Runoff:')
        self.labelAttribEvap.setText('Attribute Evapotranspiration:')